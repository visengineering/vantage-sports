Migrations: in api folder
-- Generate --

```sh
npx sequelize-cli migration:generate --name remove_user_type_from_profile
npx sequelize-cli seed:generate --name add-name-profiles
 PGSSLMODE=no-verify npx sequelize-cli --env production --config
 ../config/connection.js db:migrate:status
```

# Below old README of original starter - mostly outdated.
...
const RootQuery = new GraphQLObjectType({
  name: 'rootQuery',
  description:
    'This is the RootQuery which holds all possible READ entrypoints for the GraphQL API',
  fields: () => ({
    user: userQuery,
  }),
});
...
Do not forget to `import` the the model in `./api/models/index.ts`.
